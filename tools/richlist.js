...
const { Account } = require('../lib/serverDB.js')
const { Transaction } = require('../lib/serverDB.js')
const { Block } = require('../lib/serverDB.js')
  ...
  asyncL.waterfall(
    [
      function (callback) {
        ...
        // faster
        // dictint("from")
        Transaction.aggregate([{ $match: { blockNumber: { $lte: toBlock, $gt: fromBlock } } }, { $group: { _id: '$from' } }, { $project: { _id: 1 } }]).exec((err, docs) => {
          if (err) {
            console.log(err)
            return
          }
      ...
      function (callback) {
        // dictint("to")
        Transaction.aggregate([{ $match: { blockNumber: { $lte: toBlock, $gt: fromBlock } } }, { $group: { _id: '$to' } }, { $project: { _id: 1 } }]).exec((err, docs) => {
          if (err) {
            console.log(err)
            return
          }
      ...
      function (callback) {
        ...
        Block.aggregate([{ $match: { number: { $lte: toBlock, $gt: fromBlock } } }, { $group: { _id: '$miner' } }, { $project: { _id: 1 } }]).exec((err, docs) => {
          if (err) {
            console.log(err)
            return
          }
